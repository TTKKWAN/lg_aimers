...
scripts/pipeline.py
       |
       +--> 공통 regime 전처리              시즌 중심 대비 상대 rate
       ...
       |
       v
open/baseline_submit/model/bundle.pkl  (새 서버에서 생성)
       |
       v
...
- v13 후보는 `era_specs`를 모든 모델 앞의 공통 전처리에 적용한다. 모델 출력은 여전히
  병렬이며 서로의 예측을 다음 모델 입력으로 전달하지 않는다.
