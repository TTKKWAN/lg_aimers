...
현재 저장소에는 최종 v12 방식의 학습·추론·검증 코드만 남겨 두었습니다.
...
cp open/baseline_submit/model/bundle_v10_candidate.pkl open/baseline_submit/model/bundle.pkl
python3 scripts/build_abs_regime_final.py
cp open/baseline_submit/model/bundle_v11_abs_candidate.pkl open/baseline_submit/model/bundle.pkl
python3 scripts/build_pitcher_policy_final.py
cp open/baseline_submit/model/bundle_v12_pitcher_chase_candidate.pkl open/baseline_submit/model/bundle.pkl
```

최종 번들 생성 후 아래 검사를 수행합니다.

```bash
python3 scripts/test_submission_path.py
python3 scripts/test_catboost_submission_path.py
python3 scripts/test_pitcher_policy_submission_path.py
